...
export default function PricingMainSection() {
  ...
  useEffect(() => {
    ...
    if (expandedKeys.length > 0) {
      ...
      const sectionTypes = [
        ...new Set(expandedKeys.map((key) => key.split("-")[1])),
      ];

      setTimeout(() => {
        sectionTypes.forEach((sectionType) => {
          // Get all refs for this section type across all packages
          const sectionRefs = [];
          packages.forEach((_, packageIndex) => {
            const refKey = `${packageIndex}-${sectionType}`;
            if (
              sectionContentRefs.current[refKey] &&
              expandedSections[refKey]
            ) {
              sectionRefs.push(sectionContentRefs.current[refKey]);
            }
          });

          if (sectionRefs.length > 0) {
            // Calculate max height for this section type
            const heights = sectionRefs.map((ref) =>
              ref ? ref.scrollHeight : 0
            );
            const maxHeight = Math.max(...heights);

            // Apply height to all sections of this type
      ...
      // Reset all heights when collapsed
      Object.values(sectionContentRefs.current).forEach((ref) => {
        if (ref) {
          ref.style.height = "auto";
        }
      ...
      <div className='max-w-7xl mx-auto px-4 py-8 md:py-12'>
        <div className='grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6'>
          {packages.map((pkg, packageIndex) => (
            <div
              key={packageIndex}
              ref={(el) => (cardRefs.current[packageIndex] = el)}
